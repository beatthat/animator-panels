...
namespace BeatThat.UI
{
    ...
    public class AnimatorPanel : MonoBehaviour, Panel
    {
        ...
        class PanelAnimatorTransition : TransitionBase
        {
            ...
            {
                return "[PanelTransition owner="
                    + this.owner
                    + ", t=" + this.panelTransition
                    + ", status=" + this.status
                    + "]";
            }
            ...
            override protected void DoUpdateTransition(float time, float deltaTime)
            {
                ...
                {
                    if (time - this.startTime > this.maxTransitionTime)
                    { // we're not transitioning and we're stopped, kill
                        Cancel();
                    }
                }
            ...
            override protected void CompleteTransitionEarly()
            {
                ...
                if (!o.animator.gameObject.activeInHierarchy)
                {
#if UNITY_EDITOR || DEBUG_UNSTRIP
                    Debug.LogWarning("[" + Time.frameCount 
						+ "] cannot play animator target state on inactve game object: "
						+ o.animator.gameObject.Path());
#endif
					o.OnComplete(this);
					return;
                }
				if (!o.animator.isInitialized)
                {
#if UNITY_EDITOR || DEBUG_UNSTRIP
                    Debug.LogWarning("[" + Time.frameCount 
						+ "] cannot play animator target state on uninitialized animator: "
						+ o.animator.Path());
#endif
					o.OnComplete(this);
					return;
